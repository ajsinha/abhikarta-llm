    ...
    with AshRedisClient('localhost', 6379) as client:
        ...
        for i in range(1000000):
            ...
            if random_boolean:
                ...
                region = "test_odd"
            value = f'"Hello \\"AshRedis {i}{i}"'
            client.set(k, value, region=region)
            value = client.get(k, region=region)
            print(f"GET {k}: {value}")

        client.set('mykey', 'Hello, AshRedis!', region='test')
        value = client.get('mykey')
        ...
        # Check existence
        exists_count = client.exists('mykey', 'counter', region='test')
        print(f"Keys exist: {exists_count}")

        # Set expiration
        client.expire('mykey', 60, region='test')
        ttl = client.ttl('mykey', region='test')
