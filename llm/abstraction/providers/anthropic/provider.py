...
from .facade import AnthropicFacade
...
class AnthropicProvider(LLMProvider):
    ...
    Requires:
        ...
        - API key via environment variable ANTHROPIC_API_KEY or configuration
    """
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        self.api_key = os.environ.get('ANTHROPIC_API_KEY') or config.get('api_key')
        ...
        try:
            ...
            raise ProviderInitializationError(f"Failed to initialize Anthropic client: {str(e)}")
    
    def create_facade(self, model_name: str) -> LLMFacade:
        """
        Create Anthropic facade for the specified model.
        
        Args:
            model_name: Name of the Claude model
            
        Returns:
            AnthropicFacade instance
            
        Raises:
            ModelNotFoundError: If model is not available
        """
        if model_name not in self.list_available_models():
            raise ModelNotFoundError(f"Model '{model_name}' not found in Anthropic provider")
        
        return AnthropicFacade(self, model_name, self.client)
