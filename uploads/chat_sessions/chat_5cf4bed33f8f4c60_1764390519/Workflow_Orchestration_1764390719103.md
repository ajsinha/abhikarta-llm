Read the requirements and create backend and front end codes.  Whole code is to be packaged in a module called workflow_management. All html templates will be in afolder templates/workflow all routes will be defined in a file workflow_routes.py.  htmls will follow structure look and feel like in uploaded files. A new menu item "Workflow" will be added to the UI.  There can be only one file which has codes to interact with database. Create separate schema file for sqllite, mysql and postgres. Think harder and create bug free code .. it is taking too long and you have been making many mistakes. Create a Workflow menu item in base.html, this menu will be visible only to logged in users. All error log must also write full exception trace. Workflow can be json dag, python dag or LLM driven - use langchain and langgraph wherever possible. Create json, python and LLm workflow examples. You may ctreate a mock LLM for purpose of this code. A user can edit/delete a workflow or workflow execution that he created. An admin can do that for any workflow. This system needs to be truly top notch and next top none in industry and totally bugfree. Create 20 examples of common tools which all will extend BaseTool and will be registered in tools registry. USe DBAware baseclass in DB realted classes. All DB activities need to be confined in one class. Use an example uploaded here for model management.
Add copyright notice as below Copyright © 2025-2030, All Rights Reserved
...
Patent Pending: Certain architectural patterns and implementations described in this module may be subject to patent applications.  Create a clean, bugfree set of codes and package them ina  properly structured and modularized downloadable archive
