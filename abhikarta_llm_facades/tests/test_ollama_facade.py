...
class TestOllamaFacade(unittest.TestCase):
    """Test suite for OllamaFacade."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.model_name = "llama2"
    
    @patch('facades.ollama_facade.ollama')
    def test_initialization(self, mock_client):
        """Test facade initialization."""
        facade = OllamaFacade(model_name=self.model_name)
        
        self.assertIsNotNone(facade)
        self.assertEqual(facade.model_name, self.model_name)
        self.assertEqual(facade.provider_name, "ollama")
...
class TestOllamaFacadeIntegration(unittest.TestCase):
    """Integration tests for OllamaFacade."""
    
    @unittest.skipUnless(os.getenv("OLLAMA_HOST"), "API credentials not set")
...
def suite():
    ...
    test_suite.addTest(unittest.makeSuite(TestOllamaFacade))
    test_suite.addTest(unittest.makeSuite(TestOllamaFacadeIntegration))
    return test_suite
