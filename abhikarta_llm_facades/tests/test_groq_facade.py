...
from facades.groq_facade import GroqFacade
from facades.llm_facade import GenerationConfig, ModelCapability


class TestGroqFacade(unittest.TestCase):
    """Test suite for GroqFacade."""
    
    def setUp(self):
        """Set up test fixtures."""
        self.api_key = "test-api-key-groq"
        self.model_name = "llama2-70b-4096"
    ...
    def test_initialization(self):
        ...
        self.assertEqual(facade.provider_name, "groq")
    ...
    @patch('facades.groq_facade.Groq')
    def test_chat_completion(self, mock_client):
        """Test chat completion."""
        # Mock the response
        mock_response = Mock()
        mock_response.choices = [Mock(message=Mock(content="Test response", role="assistant"), finish_reason="stop")]
...
class TestGroqFacadeIntegration(unittest.TestCase):
    """Integration tests for GroqFacade (requires API key)."""
    ...
    @unittest.skipUnless(os.getenv("GROQ_API_KEY"), "API key not set")
...
def suite():
    ...
    test_suite.addTest(unittest.makeSuite(TestGroqFacade))
    test_suite.addTest(unittest.makeSuite(TestGroqFacadeIntegration))
    return test_suite
